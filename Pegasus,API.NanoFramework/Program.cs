...
namespace Pegasus.API
{
    public class Program
    {
...
#if HAS_WIFI
        private static string wifiSsid = "Athens House";
        private static string wifiPassword = "Athens20";
#endif
        ...
        {
            //var i2cConfig = new I2cConnectionSettings(2, Ssd1306.DefaultI2cAddress);
            //var i2cDevice = I2cDevice.Create(i2cConfig);
            //var device = new Ssd1306(i2cDevice, Ssd13xx.DisplayResolution.OLED128x64);
            //device.Font = new BasicFont();
            //device.ClearScreen();
            //device.DrawString(2, 2, "Pegasus API", 1);
            //device.Display();
            ...
            try
            {
...
#if HAS_WIFI
                success = WiFiNetworkHelper.ConnectDhcp(wifiSsid, wifiPassword, requiresDateTime: false, token: cs.Token);
#else
                success = NetworkHelper.SetupAndConnectNetwork(cs.Token, true);
#endif
                if (!success)
                {
                    Debug.WriteLine($"Can't connect to network: {WiFiNetworkHelper.Status.ToString()}.");
                    if (WiFiNetworkHelper.HelperException != null)
                    {
                        Debug.WriteLine($"Exception: {WiFiNetworkHelper.HelperException.Message}");
                    }
                    return;
                }
                ...
                var port = 80;
                var address = IPAddress.GetDefaultLocalAddress();

#if USE_TLS
                protocol = HttpProtocol.Https;
                port = 443;
#endif                
                using (WebServer server = new WebServer(port, protocol, new Type[] { typeof(PocketPowerBoxController) }))
                {
...
#if USE_TLS
                    server.HttpsCert = new X509Certificate2(_myWebServerCrt, _myWebServerPrivateKey, "1234");
                    server.SslProtocols = System.Net.Security.SslProtocols.Tls11 | System.Net.Security.SslProtocols.Tls12;
#endif
                    var protocolText = protocol == HttpProtocol.Https ? "https" : "http";

                    //device.DrawString(2, 2, $"{protocolText}://{address}");
                    Debug.WriteLine($"{protocolText}://{address}:{port}");
                    server.Start();
