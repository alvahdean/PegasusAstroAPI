...
namespace Pegasus.Driver
{
    public class PocketPowerBoxDriver : SerialBase
    {
        ...
        public void UpdateStatus()
        {
            ...
            if (values.Length !=12 || values[0] !="PPB")
            {
            ...
            State.Current = MyRound(double.Parse(values[i++])/65d,1);
            ...
            State.DewHeaterPowerA = MyRound(int.Parse(values[i++]) / 255d, 1);
            State.DewHeaterPowerB = MyRound(int.Parse(values[i++]) / 255d, 1);
        ...
        private static double MyRound(double value, int digits = 0)
        {
#if NANOFRAMEWORK_1_0
            var factor = 10^digits;
            var adjValue = value * factor;
            var rounded=System.Math.Round(adjValue);
